  ...
  return (
    <>
      <div className="flex justify-center h-screen bg-blue-400 overflow-hidde p-16">
        <div className="bg-white w-1/3 shadow-xl flex flex-col rounded-2xl overflow-hidden">
          <header className="h-16 bg-white border-b border-gray-200"></header>
          <div
            ref={scrollBar}
            onScroll={() => onScrollHandler()}
            className={["relative flex-shrink-1 flex h-full overflow-y-scroll flex-col-reverse p-2"].join(" ")}
          >
            {chats.slice(0, displayNumber).map((each, eachIndex) =>
              arrayReverseObj(each.message).map((chat, chatIndex) => (
                <div className={["mx-2 flex items-end my-4",isMe(eachIndex) && "flex-row-reverse"].join(" ")}>
                {isBottom(chatIndex) && (
                  <div className="w-8 h-8 mx-2 bg-gray-300 rounded-full">
                ...
                <p
                  ...
                  className={[
                    ...
                    isMe(eachIndex)
                      ? "bg-blue-500 text-right rounded-br-none text-white"
                      : "bg-gray-100 text-left rounded-bl-none text-gray-900",
                  ].join(" ")}
                >
                  {chat.text}
                </p>
